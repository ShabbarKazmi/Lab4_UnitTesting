...
namespace Lab4_UnitTesting
{
    public class Tests
    {
        ...
        {
            String InvalidAnswer = "I dont know if we are even going to have a clue which is " +
               " going to be above 250 characters but it is one of the tests that I have not done so why not try it right ? " +
               "Hopefully I wont get docked for having such a long text as it might not be on one line and will affect readability";

            BL.AddEntry("Test", "Test", 1, "19\\10\\2022");

            EntryEditError editError = BL.EditEntry(InvalidAnswer, "no eye deer", 1, "20\\1\\2022", BL.GetEntries().Last().Id);
        ...
        public void UpdateInvalidDifficultyTest()
        {
            ...
            EntryEditError editError = BL.EditEntry("deer with no eyes who also cannot see and I hope this is actually ","no eye deer",-1,"20\\1\\2022", BL.GetEntries().Last().Id);
            Assert.That(editError, Is.EqualTo(EntryEditError.InvalidFieldError));
        }
